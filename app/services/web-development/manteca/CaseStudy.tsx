...
import { ArrowRight, Search, Smartphone, Calendar, TrendingUp } from "lucide-react";
  ...
  return (
    <section id="case-study" data-fade className="scroll-mt-20">
      ...
      <h2 className="font-display text-3xl sm:text-4xl md:text-[2.75rem] font-bold text-warm-white leading-[1.2] tracking-tight text-balance mb-4 max-w-2xl">
        How a new Manteca business started <span className="text-[#004D3A]">getting found.</span>
      </h2>
      <p className="text-base sm:text-lg text-stone-600 leading-relaxed max-w-2xl mb-12">
        My Massage Cottage came to me with zero online presence and an opening day approaching fast.
        Here&apos;s what I built — and what&apos;s happening now.
      </p>
      ...
      >
        <div className="flex items-center gap-3 px-4 py-2 border-b border-white/[0.04]">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-md bg-white/[0.03] border border-white/[0.04]">
            <span className="text-[10px] text-white/25 font-mono">mymassagecottage.com</span>
          </div>
        </div>
        <Image
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-full border border-[#004D3A]/20 bg-[#004D3A]/[0.05] flex items-center justify-center flex-shrink-0">
                <step.icon className="w-4 h-4 text-[#004D3A]" aria-hidden="true" />
              </div>
              <div>
                <p className="text-[10px] font-mono text-stone-500 uppercase tracking-widest">
                  Step {i + 1} — {step.label}
                </p>
                <h3 className="text-[15px] font-semibold text-stone-900">{step.title}</h3>
              </div>
            </div>
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 rounded-xl border border-stone-200 bg-stone-50 p-6 sm:p-8">
        <div>
          <p className="text-sm font-medium text-stone-900 mb-2">The result so far:</p>
